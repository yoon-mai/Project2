...
export default function RichNoteEditor({value,onChange,notes,onOpenNote}:{value:string;onChange:(v:string)=>void;notes:string[];onOpenNote:(name:string)=>void}){
  ...
  function image(file?:File){if(!file)return;const reader=new FileReader();reader.onload=()=>cmd("insertImage",String(reader.result));reader.readAsDataURL(file);}
